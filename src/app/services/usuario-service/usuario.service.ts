...
export class UsuarioService {
  ...
  getLocation(idUsuario: string): Observable<any> {
    ...
    if (cache[idUsuario]) {
      ...
      const url = 'http://13.38.223.212:8003/api/coordenadas/' + idUsuario + '/';
  ...
  getUsuarioInfo(idUsuario: string): Observable<any> {
    const url = 'http://13.38.223.212:8000/api/usuarios/' + idUsuario + '/';
    return this.http.get<any>(url);
  }

  getUsuarioInfoPorMail(email: string): Observable<any> {
    const url = 'http://13.38.223.212:8000/api/usuarios/email/' + email + '/';
    return this.http.get<any>(url);
  }
  ...
  createUsuario(usuario: Usuario): Observable<Usuario> {
    const url = 'http://13.38.223.212:8000/api/usuarios/';
